...
def mean(x):
    ...
    return sumx/count
#
def publishResults(s_count, count, net_count, word_length, xx2):
    ...
    sentence_avg = count/s_count
    # FKRA = (0.39 x ASL) + (11.8 x ASW) - 15.59 
    fkra = (0.39*sentence_avg) + (11.8*1.65) - 15.59
    h.write("<span style = \"text-align: center; font-size: 16px; color: navy;\">Text Analysis (Reading level)</span><br/>")
    h.write("<p>File: "+xfile+".doc</p>")
    h.write("<p>Sentence count: "+str(s_count)+", Average words: "+str(round(sentence_avg,1))+"</p>")
    h.write("<p>Total Word count: "+str(count)+"</p>")
    h.write("<p>Net Word count: "+str(net_count)+"</p>")
    h.write("<p>Unique words: "+str(k)+" -- % of net: "+str(round(ratio_unique,2))+"% </p>")
    h.write("<p>Average word length (characters): "+str(round(word_length,1))+"</p>")
    h.write("<p>Reading Level - grade: "+str(round(fkra,1))+"</p>")
    h.write("<table cellspacing = \"0\" cellpadding = \"1\" border=\"1\" width = \"500\">")
   ...
   #
    ...
    for i in range(k):
         if (word_instance[i] > "aa" and word_count[i] > float(limit)) :
             ...
             h.write("<td width = \"50\" align=\"right\">"+str(word_count[i])+"</td>")
...
def main():
    ...
    xx2 = xx1.split(" ")
    net_count = len(xx2)
    publishResults(s_count,count,net_count, word_length,xx2)
    #
