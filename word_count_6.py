...
def publishResults(sentence_count, count, net_count, word_length, xx2):
   ...
   f.write("<html>\n")
   f.write("<body width=\"90%\">")
   f.write("<div style=\"display:block; width: 800px;\">")
   #
   h = open(xfile+"_stats.html","w+")
   h.write("<html>\n")
   h.write("<body>\n")
   j = 1
   k = 0
   ...
   #
   h.write("<span style = \"text-align: center; font-size: 16px; color: navy;\">Text Analysis (Reading level)</span><br/>")
   h.write("<p>Input file: "+xfile+".doc</p>")
   h.write("<ul>")
   h.write("<li>Sentence count: "+str(sentence_count)+", Average words per sentence: "+str(round(sentence_avg,1))+"</li>")
   h.write("<li>Total Word count: "+str(count)+"</li>")
   h.write("<li>Net Word count (less stop words): "+str(net_count)+"</li>")
   h.write("<li>Unique words: "+str(k)+" -- % of Net: "+str(round(ratio_unique,2))+"% </li>")
   h.write("<li>Average word length (characters): "+str(round(word_length,1))+"</li>")
   h.write("<li>Average number of syllables per word: "+str(round(average_syllable,2))+"</li>")
   h.write("<li>FKRA (Flesch-Kincaid) Reading Level - Grade: "+str(round(fkra,1))+"</li>")
   h.write("<li>FKRE (Flesch-Kincaid) Reading Level - ease: "+str(round(fkre,1))+"</li>")
   h.write("<li>Sentences per 100 words: "+str(round(sentence_hundred,1))+", Syllables per 100 words: "+str(round(syllables_hundred,1))+"</li>")
   h.write("</ul>")
   #
